...
describe('231021 문제풀이', () => {
    ...
    describe('공배수', () => {
        ...
        ('number(%s)가 n(%s)의 배수이면서 m(%s)의 배수이면 %s을 return', (number, n, m, expected) => {
    ...
    describe('문자열 앞의 n글자 ', () => {
        ...
        ('$my_string 앞의 $n 글자로 이루어진 $expected 를 return', ({my_string, n, expected}) => {
    ...
    describe('특정한 문자를 대문자로 바꾸기', () => {
        ...
        ('$my_string 에서 모든 $alp 를 대문자로 변환', ({my_string, alp, expected}) => {
            expect(solution.convertSpecificCharToUpperCase(my_string, alp)).toBe(expected);
        });
    });
});