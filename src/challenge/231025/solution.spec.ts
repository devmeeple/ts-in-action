...
describe('231025 문제풀이', () => {
  ...
  describe('수 조작하기1', () => {
    it.each([{ n: 0, control: 'wsdawsdassw', expected: -1 }])(
      '가장 마지막에 나오는 n의 값은 $expected',
      ({ n, control, expected }) => {
        expect(solution.manipulateNumberByControlString(n, control)).toBe(
          expected,
        );
      },
    );
  ...
  describe('배열 만들기 1', () => {
    it.each([
      ...
      '1이상 $n 이하의 정수 중에서 $k의 배수를 오름차순으로 저장한 배열 $expected',
      ({ n, k, expected }) => {
