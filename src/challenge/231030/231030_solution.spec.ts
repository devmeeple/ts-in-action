...
describe('231030 문제풀이', () => {
  describe('접미사인지 확인하기', () => {
    it.each([
      ...
      '$is_suffix가 $my_string에 포함여부 $expected를 반환한다.',
      ({ my_string, is_suffix, expected }) => {
        // given
  ...
  describe('접두사인지 확인하기', () => {
    it.each([{ my_string: 'banana', is_prefix: 'ban', expected: 1 }])(
      '접두사라면 1, 아니면 0을 반환한다.',
      ({ my_string, is_prefix, expected }) => {
        // given
  ...
  describe('글자 이어 붙여 문자열 만들기', () => {
    it.each([
      ...
      '인덱스의 글자들을 순서대로 이어 붙인 문자열을 반환한다.',
      ({ my_string, index_list, expected }) => {
        // given
