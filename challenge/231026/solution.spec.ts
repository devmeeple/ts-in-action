...
describe('231026 문제풀이', () => {
    describe('n 번째 원소부터', () => {
        ...
        ])('$num_list의 $n번째 까지 담은 배열은 $expected', ({num_list, n, expected}) => {
            expect(solution.getElementsFromNthToEnd(num_list, n)).toStrictEqual(expected);
        });
    });
    ...
    describe('정수 찾기', () => {
        ...
        ])('$num_list에서 $n이 포함여부: $expected', ({ num_list, n, expected}) => {
    ...
    describe('마지막 두 원소', () => {
        it.each([
            ...
            expect(solution.appendModifiedLastElement(num_list)).toStrictEqual(expected);
        });
    });
});